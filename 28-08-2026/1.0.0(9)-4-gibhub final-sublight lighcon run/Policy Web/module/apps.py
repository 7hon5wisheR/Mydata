...
# ======================================================
# SHUTDOWN
# -- Mirrors reboot_after_delay() exactly in shape/timing, but powers
#    the RPi OFF instead of restarting it. Unlike reboot, there is no
#    automatic recovery: the unit stays off until someone physically
#    reconnects power, so this must only ever be triggered by an
#    explicit, confirmed user action (see module.py's confirm() dialog
#    and apps.py's on_message()/cmd=="SHUTDOWN" targeting logic below).
# ======================================================
def shutdown_after_delay(delay=3):
    time.sleep(delay)
    ...
    try:
        ...
        c.loop(timeout=1.0)
        c.disconnect()
        print(f"[SHUTDOWN] Published to broker={broker_ip} topic='in': {payload}")

    except Exception as e:
        print(f"[SHUTDOWN] Failed to publish shutdown notice: {e}")

    print("[SHUTDOWN] Waiting 5s before power-off...")
    time.sleep(5)
    print("[SHUTDOWN] Executing sudo shutdown -h now ...")
...
def on_message(client, userdata, msg):
    ...
    try:
        ...
        # ------------------------------------------------------------
        # Targeting filter - shared by everything below this point,
        # INCLUDING the SHUTDOWN command handled further down.
        #
        # device_in_msg == ""      -> broadcast to everyone (same rule
        #                              as every other field on this side)
        # device_in_msg == "all"   -> broadcast to everyone (explicit)
        # device_in_msg == DEVICE_MAC -> for this cabinet only
        # anything else           -> not for this cabinet, ignore
        #
        # This is exactly what makes "which cabinet reacts to a
        # shutdown" work the same way ip_broker/hostname/etc already
        # do: module.py always sends ONE MQTT message with "mac" set
        # to either a specific cabinet_id or "all"; every apps.py
        # instance receives it (same "in" topic) but only the matching
        # one(s) proceed past this check.
        # ------------------------------------------------------------
        ...
        # ------------------------------------------------------------
        # SHUTDOWN command - mirrors REBOOT above, but ALSO honors
        # device_in_msg == "all" (broadcast power-off to every
        # cabinet), which REBOOT deliberately does not offer. The
        # generic targeting filter earlier in this function has
        # already thrown away any message not meant for this cabinet
        # (mac not in ("", "all", DEVICE_MAC)), so by the time we get
        # here it's safe to act unconditionally on cmd=="SHUTDOWN".
        # ------------------------------------------------------------
        if payload.get("cmd", "").upper() == "SHUTDOWN":
            print(f"[SHUTDOWN] cmd received (target mac in payload='{device_in_msg}') -> executing")
...
# ======================================================
# API: SHUTDOWN
# Direct REST trigger for THIS unit only (mirrors /api/reboot). Used
# by module.py's reboot_rpi()-style direct call when the target is a
# single specific cabinet reachable at its own :8082. Broadcasting a
# shutdown to "all" cabinets goes through MQTT (see
# publish_shutdown_to_mqtt() in module.py) instead, since REST here
# can only ever reach one cabinet at a time (the one this request was
# sent to).
# ======================================================
@app.route("/api/shutdown", methods=["POST"])
def api_shutdown():
    print("[SHUTDOWN] Shutdown command received via REST")
    threading.Thread(target=shutdown_after_delay, args=(3,), daemon=True).start()
    return jsonify({"status": "ok", "message": "Shutting down in 3 seconds..."})
...
if __name__ == "__main__":
    ...
    if not broker_ip:
        ...
        try:
            # force=True: sama seperti klik tombol "Set" di index.html -
            # selalu submit ulang bridge.conf + queue/binding + broadcast
            # ip_broker walaupun IP-nya tidak berubah dari sebelumnya,
            # supaya tiap restart/reboot dijamin ke-setup ulang.
