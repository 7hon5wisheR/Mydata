...
#
# FILE LOCATIONS:
#   This module lives in broker/library/sublight.py, but config.json
#   and status.json live one directory UP, at broker/config.json and
#   broker/status.json (sibling of the library/ folder) - matching
#   how apps.py (which also lives at broker/) resolves them. PROJECT_ROOT
#   below is computed as the parent of this file's directory so both
...
#
# PWM READBACK NOTE:
#   RPi.GPIO's software PWM is write-only - ChangeDutyCycle() has no
#   matching "read current duty" call. _last_duty below tracks the
#   last value this module itself commanded, exposed via
#   get_current_pwm(), for testing/debugging (see test_pwm.py).

__version__ = "1.0.0(10) - fixed config.json/status.json path resolution"
...
#      Updated                    : 27-08-2026
#      Changes                    : Fixed CONFIG_PATH/STATUS_PATH - they were
#                                   resolving to library/config.json instead of
#                                   the actual broker/config.json (one directory
#                                   up from this file). Added get_current_pwm().
#      Author                     : Saifuddin
# ======================================================================================================================================
...
# This file lives at broker/library/sublight.py.
# config.json and status.json live at broker/ (one level up).
LIBRARY_DIR  = os.path.dirname(os.path.abspath(__file__))   # .../broker/library
PROJECT_ROOT = os.path.dirname(LIBRARY_DIR)                 # .../broker

CONFIG_PATH = os.path.join(PROJECT_ROOT, "config.json")
STATUS_PATH = os.path.join(PROJECT_ROOT, "status.json")   # EXISTING file, reused here
...
# Last duty cycle this module commanded via set_pwm(). Software-level
# ground truth only (see PWM READBACK NOTE above) - not read from
# hardware, since software PWM can't be read back.
_last_duty = 0
...
def get_current_pwm() -> int:
    """
    Return the last duty cycle (0-100) this module commanded via
    set_pwm(). This is a SOFTWARE-level record of what was sent to
    ChangeDutyCycle() - RPi.GPIO's software PWM has no hardware
    readback, so this does NOT measure the electrical signal itself.
    For that, use test_pwm.py's empirical GPIO.input() sampling.
    """
    return _last_duty
