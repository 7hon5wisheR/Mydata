...
# ======================================================
# READINESS FLAG (consumed by send.py)
# ======================================================
# send.py starts apps.py first, then waits for this flag file to
# appear before starting run.py / radarScanControl / lightLockControl /
# switchStatus - all of which talk MQTT and should only come up once
# the broker side (bridge.conf, mosquitto restart if needed, queue +
# binding) is actually settled, not just after a guessed sleep().
# write_ready_flag() is called once, at the very end of the __main__
# startup sequence below, right before serve() takes over the process.
APPS_READY_FLAG = os.path.join(BASE_DIR, "apps_ready.flag")

def write_ready_flag():
    try:
        with open(APPS_READY_FLAG, "w") as f:
            f.write(str(time.time()))
        print(f"[READY] apps.py ready flag written: {APPS_READY_FLAG}")
    except Exception as e:
        print(f"[READY] Failed to write ready flag: {e}")
...
if __name__ == "__main__":
    ...
    # Clear any stale ready flag from a previous run right away, so
    # send.py (which polls for this flag) never mistakes a leftover
    # flag for "this run is ready" before we've actually finished
    # setting up the broker/bridge/queue below.
    try:
        if os.path.exists(APPS_READY_FLAG):
            os.remove(APPS_READY_FLAG)
    except Exception as e:
        print(f"[READY] Could not remove stale ready flag: {e}")
    ...
    # ---- Everything above this line is the "startup work" send.py is
    # waiting on: broker detected/applied, bridge.conf written +
    # mosquitto restarted (if needed), queue/binding ensured. Only NOW
    # do we signal readiness, right before handing off to serve() -
    # this is what send.py's wait_for_apps_ready() polls for instead
    # of a guessed time.sleep(N).
    write_ready_flag()
