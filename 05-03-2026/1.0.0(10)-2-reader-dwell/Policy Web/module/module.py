...
#
# Mirrors publish_ip_broker_to_mqtt() in shape, but for a one-shot
# power-off command instead of a config value. Every apps.py process
# on every cabinet is subscribed to the same "in" topic, so this
# message reaches all of them - it's apps.py's own on_message()
# filter (device_in_msg not in ("all", DEVICE_MAC) -> return) that
# narrows it down to only the intended cabinet(s).
#
# IMPORTANT: target_mac must NEVER be empty ("") - apps.py's filter
# treats an empty/missing "mac" the same as "all" (broadcast to
# everyone), so callers must always pass either a specific cabinet_id
# (e.g. "MASCAB01A") or the literal string "all".
# ============================================================

def publish_shutdown_to_mqtt(target_mac, broker_ip):
    """
    target_mac : specific cabinet_id (e.g. "MASCAB01A") to shut down
                 only that unit, or the literal string "all" to shut
                 down every cabinet currently listening on this broker.
    broker_ip  : the RabbitMQ/Mosquitto broker IP to publish through.

    Returns (ok: bool, message: str).
    """
    target_mac = (target_mac or "").strip()
    if not target_mac:
        # Defensive guard - never let an empty mac slip through, since
        # apps.py's on_message() treats "" the same as "all".
        return False, "target_mac must not be empty (use a cabinet_id or 'all')"

    try:
        c = mqtt.Client()
        c.connect(broker_ip, BROKER_PORT, 60)
        c.loop_start()
        msg = json.dumps({"cmd": "SHUTDOWN", "mac": target_mac}, separators=(',', ':'))
        c.publish("in", msg).wait_for_publish()
        c.loop_stop()
        c.disconnect()
        print(f"[MQTT PUBLISH] shutdown cmd (mac={target_mac}) published to 'in'")
        return True, "Shutdown command published"
    except Exception as e:
        print(f"[MQTT PUBLISH] Failed to publish shutdown: {e}")
        return False, str(e)
...
def send_command():
    ...
    # ── shutdown ────────────────────────────────────────────
    # "value" carries the target scope from module.html's dedicated
    # Shutdown dropdown:
    #   "unit" -> shut down only the currently-selected cabinet (mac)
    #   "all"  -> shut down every cabinet currently on this broker
    #
    # Sent over MQTT (topic "in") exactly like reboot's underlying
    # mechanism, but reboot uses a direct REST call to that one RPi's
    # /api/reboot, whereas shutdown is broadcast over MQTT so a single
    # message can target "all" cabinets in one shot. Every apps.py
    # instance subscribes to the same "in" topic; apps.py's own
    # on_message() filter (mac not in ("all", DEVICE_MAC) -> ignore)
    # is what makes sure only the intended cabinet(s) actually act on
    # it - the rest silently ignore the message.
    if option == "shutdown":
        target = (value or "unit").strip().lower()   # "unit" | "all"

        if target == "all":
            target_mac = "all"
            label = "all cabinets"
        else:
            # "unit" (or anything else unexpected) -> treat as this
            # cabinet only, using the cabinet currently selected in
            # module.html (the "mac" field of the request).
            target_mac = mac
            label = mac

        ok, msg = publish_shutdown_to_mqtt(target_mac, CURRENT_BROKER)
        if ok:
            print(f"[SHUTDOWN] cmd published -> target={label}")
            return jsonify({"status": "ok", "message": f"Shutdown command sent to {label}"})
        else:
            print(f"[SHUTDOWN] publish FAIL -> {msg}")
            return jsonify({"status": "error", "message": msg}), 500
