# -*- coding: utf-8 -*-
__version__ = "1.0.0(11) - per-tag RSSI + per-antenna breakdown dumped to rssi.json every scan cycle"
...
#      Name                       : READER
#      Version                    : 1.0.0(11)
#      Date Update                : 04-09-2026
#      Changes vs 1.0.0(10)       :
#        1) NEW: per-tag RSSI + per-antenna breakdown, dumped to
#           RSSI_JSON_PATH ("rssi.json", same folder as config.json - one
#           level up from wherever reader.py itself lives) at the end of
#           EVERY scan cycle (overwritten, not appended - always reflects
#           the most recent scan). Not an EX10 protocol feature, purely a
#           reader.py-side diagnostic export. Answers two questions that
#           a flat EPC list can't:
#             - "Tag ini RSSI-nya berapa?" -> per-tag "rssi_best" (best
#               single reading all-antenna, already tracked for the
#               existing RSSI ghost-reject step) PLUS a full per-antenna
#               breakdown, not just one aggregate number.
#             - "Kalau tag kebaca di antena 1, apa antena 2 juga baca tag
#               yang sama?" -> per-tag "antennas" dict lists EVERY
#               antenna that detected it this cycle; more than one key
#               means multiple antennas saw the same physical tag
#               (antenna coverage overlap), directly visible per-tag.
#           New tracking dict epc_antenna_rssi = {epc: {ant: best_rssi}},
#           populated in the same per-tag read loop that already tracks
#           epc_best_rssi/epc_native_max_count - no new parsing needed,
#           ANT/RSSI were already being extracted by
#           parse_fast_mode_correct() (METADATA_FLAGS=0x003F has always
#           requested both). Only PUBLISHED tags (post ghost-filter, same
#           set as result["epc_seen"]) are written, so rssi.json always
#           matches what's actually reported downstream. Written
#           atomically (write to .tmp then os.replace()) so a crash
#           mid-write never leaves a corrupt/partial rssi.json behind.
#
#      Version                    : 1.0.0(10)
#      Date Update                : 04-09-2026
#      Changes vs 1.0.0(9)        :
#        1) NEW: automatic SET POWER step-down fallback
#           (send_power_command_with_fallback()). Doc sec 10.2/Appendix 1:
#           an FCC-certified module caps at 30dBm (status 0x0105
#           "Parameter value not available" if asked for more), while a
#           CHINA-certified module allows up to 33dBm - config.json has
#           no per-region field, so a config written for a CHINA unit
#           (power=33) gets silently rejected on an FCC unit. Now tries
#           the requested dBm first, and if rejected, steps down through
#           30/28/27/26/25 until the module accepts one - config.json is
#           NOT modified, but the module is no longer left holding an
#           unconfirmed/possibly-stale power setting. Accepted value is
#           cached per antenna-set for the life of the process so later
#           cycles go straight to the known-good value.
#        2) NEW: automatic SET ANTENNA DWELL step-down fallback
#           (send_dwell_command_with_fallback()). Doc sec 7.3: dwell time
#           and the frequency hopping table are the SAME command (0x95).
#           Observed on an FCC-certified unit: a dwell value that is
#           accepted fine on a CHINA unit (e.g. 800ms, auto-tuned for 5
#           antennas @ scan_timeout=12s) is REJECTED with status 0x0500
#           "Frequency value not available" (Appendix 1) - almost
#           certainly the FCC unit's mandatory frequency-hopping timing
#           constraint, which a CHINA unit doesn't enforce. The doc does
#           not publish an exact per-region dwell ceiling, so rather than
#           hardcode a guessed number, this steps down through a
#           descending ladder (500/400/300/250/200/150/100/75/50/20ms)
#           until the module accepts one. If AUTO_TUNE is on and the
#           accepted dwell differs from the ideal one, one_cycle_s and
#           QUIET_NEW_TAG (quiet-stop) are recomputed against the REAL
#           accepted dwell so the scan loop's stop condition stays
#           consistent with what the module is actually doing - not the
#           value originally requested. If the module rejects every
#           candidate, the command is skipped that cycle and the module
#           keeps its own factory default (4000ms/antenna, doc p.133).
#           config.json is NOT modified by either of these fallbacks -
#           both are purely runtime/in-process behavior.
#
#      Version                    : 1.0.0(9) 
...
#           10-12s scan (dup ratios 3-5x, per log2.txt); a "ghost" - RF bleed
#           from a neighbouring cabinet/compartment, a stray tag near the
#           antenna, an occasional corrupted-but-CRC-valid read - is far
#           more likely to be seen only once or twice in the whole
#           window. This complements remove_confirm_cycles (which
...
# =====================================================
#  SET POWER - automatic step-down fallback - NEW in 1.0.0(10)
#
#  Doc sec 10.2 (0xAA04 notes, p.214): "The power value must be within the
#  range allowed by the certification bank. For example, if the
#  certification bank is FCC, the maximum power can only be 30dBm." This
#  module's own POWER_MAX_DBM=33 above is correct for a CHINA-certified
#  unit, but NOT for an FCC-certified one - config.json has no per-region
#  awareness (it's shared across every reader.py deployment, whatever the
#  hardware's actual certification region is), so a config.json written
#  for a CHINA unit (power=33) will get REJECTED by an FCC unit with
#  status 0x0105 "Parameter value not available" (Appendix 1, p.234).
#
#  Rather than require a separate config.json per region (or hard-fail
#  the whole scan cycle), this steps DOWN through a descending ladder of
#  known-safe dBm values and uses the highest one the module actually
#  accepts THIS session - fully automatic, config.json untouched. The
#  accepted value is cached (module_max_power_cache) so every later scan
#  cycle in the same process goes straight to the last-known-good value
#  first, instead of re-discovering it via trial and error every time.
# =====================================================
_module_max_power_cache = {}  # {ant_tuple: accepted_dbm} - process-lifetime cache


def send_power_command_with_fallback(ser, ant_list, requested_dbm, send_fn):
    """
    Tries `requested_dbm` first; if the module rejects it (any non-zero
    status, e.g. 0x0105 on an FCC unit asked for >30dBm), steps down
    through POWER_FALLBACK_LADDER_DBM until one is accepted, or gives up
    at POWER_MIN_DBM. Returns (accepted_dbm_or_None, ok_bool).
    `send_fn` is send_command_checked, passed in to avoid a forward-ref.
    """
    ant_key = tuple(sorted(set(ant_list)))
    cached = _module_max_power_cache.get(ant_key)
    candidates = []
    if cached is not None and cached < requested_dbm:
        candidates.append(cached)  # try the last-known-good value first
    candidates.append(requested_dbm)
    for step in (30, 28, 27, 26, 25):
        if step < requested_dbm and step not in candidates:
            candidates.append(step)
    # try highest first (cached value or requested), then descend
    candidates = sorted(set(c for c in candidates if POWER_MIN_DBM <= c <= POWER_MAX_DBM), reverse=True)
    if cached is not None and cached in candidates:
        candidates.remove(cached)
        candidates.insert(0, cached)

    for dbm in candidates:
        cmd = build_power_command(ant_list, dbm)
        ok = send_fn(ser, cmd, "SET POWER (%ddBm%s)" % (dbm, " requested" if dbm == requested_dbm else " fallback"), delay=0.15)
        if ok:
            if dbm != requested_dbm:
                log.warning(
                    "[CONFIG] SET POWER: requested %ddBm was REJECTED by the "
                    "module (likely an FCC-certified unit capped at 30dBm per "
                    "doc sec 10.2/Appendix 1 status 0x0105 - config.json's "
                    "power=%d was written for a CHINA-range unit). Falling "
                    "back to %ddBm, which the module DID accept - antenna "
                    "output will be %ddBm this run, not the configured %ddBm. "
                    "config.json was not modified.",
                    requested_dbm, requested_dbm, dbm, dbm, requested_dbm
                )
            _module_max_power_cache[ant_key] = dbm
            return dbm, True

    log.error(
        "[CONFIG] SET POWER: module rejected EVERY candidate dBm tried "
        "(%s) - could not confirm any power level. Module keeps whatever "
        "power was last successfully applied (could be a previous cycle's "
        "value, or its own hardware default).", candidates
    )
    return None, False
...
# =====================================================
#  ANTENNA DWELL - automatic step-down fallback - NEW in 1.0.0(10)
#
#  Doc sec 7.3 titles command 0x95 as ONE command covering THREE things:
#  "Set the frequency hopping table, antenna dwell time, and duty cycle"
#  (p.133) - dwell time (Option=0x02) and the frequency hopping table
#  (no Option byte) are the SAME command code, just different Data
#  layouts. On a CHINA-certified unit (no mandatory frequency-hopping
#  regulation) an 800ms dwell is accepted without issue. On an
#  FCC-certified unit (frequency-hopping IS mandatory under FCC Part
#  15.247), the observed behavior is that the same 800ms dwell is
#  rejected with status 0x0500 "Frequency value not available"
#  (Appendix 1, p.235) - the doc does not publish an exact numeric dwell
#  ceiling per region, so rather than hardcode a guessed number, this
#  steps DOWN through a descending ladder of candidate dwell values and
#  uses the highest one the module actually accepts. If the module
#  rejects EVERY candidate, this command is not sent at all that cycle
#  and the module keeps using its own factory default (4000ms/antenna,
#  doc p.133: "if it is 0, the default is 4 seconds").
#  Like the power fallback above, the accepted value is cached per
#  process so later cycles try the last-known-good value first.
# =====================================================
_module_max_dwell_cache = None  # accepted dwell_ms from a previous cycle this process, or None


def send_dwell_command_with_fallback(ser, ideal_dwell_ms, send_fn):
    """
    Tries `ideal_dwell_ms` first (the value auto-tuning/config actually
    wants); if rejected, steps down through a descending ladder until one
    is accepted, or gives up (module falls back to its own 4000ms/antenna
    default). Returns (accepted_dwell_ms_or_None, ok_bool).
    """
    global _module_max_dwell_cache

    ideal_dwell_ms = int(max(DWELL_MIN_MS, min(DWELL_MAX_MS, ideal_dwell_ms)))

    candidates = []
    if _module_max_dwell_cache is not None and _module_max_dwell_cache < ideal_dwell_ms:
        candidates.append(_module_max_dwell_cache)
    candidates.append(ideal_dwell_ms)
    for step in (500, 400, 300, 250, 200, 150, 100, 75, 50, DWELL_MIN_MS):
        if step < ideal_dwell_ms and step not in candidates:
            candidates.append(step)
    candidates = sorted(set(c for c in candidates if DWELL_MIN_MS <= c <= DWELL_MAX_MS), reverse=True)
    if _module_max_dwell_cache is not None and _module_max_dwell_cache in candidates:
        candidates.remove(_module_max_dwell_cache)
        candidates.insert(0, _module_max_dwell_cache)

    for dwell_ms in candidates:
        cmd = build_dwell_command(dwell_ms)
        ok = send_fn(ser, cmd, "SET ANTENNA DWELL (%dms%s)" % (
            dwell_ms, " requested" if dwell_ms == ideal_dwell_ms else " fallback"
        ), delay=0.15)
        if ok:
            if dwell_ms != ideal_dwell_ms:
                log.warning(
                    "[CONFIG] SET ANTENNA DWELL: requested %dms was REJECTED "
                    "by the module (status 0x0500 'Frequency value not "
                    "available', Appendix 1 - dwell time shares command 0x95 "
                    "with the frequency hopping table, doc sec 7.3; likely an "
                    "FCC-certified unit's frequency-hopping timing "
                    "constraint, which a CHINA-certified unit does not "
                    "enforce). Falling back to %dms, which the module DID "
                    "accept. Round-robin timing this run uses %dms/antenna, "
                    "not the auto-tuned/configured %dms. config.json was not "
                    "modified.",
                    ideal_dwell_ms, dwell_ms, dwell_ms, ideal_dwell_ms
                )
            _module_max_dwell_cache = dwell_ms
            return dwell_ms, True

    log.error(
        "[CONFIG] SET ANTENNA DWELL: module rejected EVERY candidate dwell "
        "tried (%s) - command not sent this cycle. Module keeps using its "
        "own factory default (4000ms/antenna per doc p.133) or whatever "
        "dwell was last successfully applied.", candidates
    )
    return None, False
...
# NEW in 1.0.0(11): per-tag RSSI/antenna diagnostic dump - see
# write_rssi_json() near the end of run_async_scan() for what goes in it.
# Same folder as config.json (BASE_DIR = one level up from wherever
# reader.py itself lives, e.g. reader.py in .../broker/library/reader.py
# -> RSSI_JSON_PATH = .../broker/rssi.json, "sejajar" with the library
...
# =====================================================
#  RSSI/ANTENNA DIAGNOSTIC DUMP - NEW in 1.0.0(11)
#
#  Not an EX10 protocol feature - purely a reader.py-side diagnostic
#  file, written fresh every scan cycle (overwritten, not appended) so
#  it always reflects the MOST RECENT scan. For every published tag,
#  records the best (least-negative) RSSI seen from EACH antenna that
#  detected it this cycle - this directly answers two things a plain
#  EPC list can't:
#    1) "Tag ini kebaca RSSI berapa?" - epc's "rssi_best" plus a full
#       per-antenna breakdown, not just one aggregate number.
#    2) "Kalau tag kebaca di antena 1, apa antena 2 juga baca tag yang
#       sama?" - epc's "antennas" dict lists EVERY antenna that saw it;
#       more than one key means multiple antennas detected the same
#       physical tag this cycle (overlap between antenna coverage
#       areas), which a single flat EPC-count list has no way to show.
#  Saved as RSSI_JSON_PATH = one level up from wherever reader.py itself
#  lives (same folder as config.json - "sejajar" with the library
#  folder, not inside it).
# =====================================================
...
def run_async_scan(baud, stop_event=None):
    ...
    try:
        ...
        log.warning(
            ...
            "auto-tuning/power.", ant_list_now
        )

    try:
        requested_power_dbm = int(str(cfg.get("power", "")).strip())
    except (ValueError, TypeError):
        requested_power_dbm = None  # get_power_command() already validated this at startup;
                                     # if config.json is somehow broken mid-run, just skip the fallback logic
    ...
    if AUTO_TUNE:
        ...
        quiet_cycles_used = quiet_cycles  # kept for the post-fallback QUIET_NEW_TAG recompute below
    else:
        dwell_cmd, ANTENNA_DWELL = get_dwell_command(cfg)
        QUIET_NEW_TAG = float(cfg.get("scan_newtag", 10))
        quiet_cycles_used = None  # manual mode: QUIET_NEW_TAG comes straight from scan_newtag,
                                   # not derived from dwell, so no recompute needed if dwell falls back
        log.info(
    ...
    ideal_dwell_ms = ANTENNA_DWELL  # the plan before hardware negotiation - see the
                                     # SET ANTENNA DWELL block below, where this may be
                                     # revised downward if the module rejects it.
    ...
    epc_antenna_rssi = {}      # NEW in 1.0.0(11): {epc: {ant: best_rssi_on_that_antenna}} -
                               # see write_rssi_json() for what this feeds into
    buffer    = bytearray()
    ...
    try:
        with serial.Serial(PORT, baudrate=baud, timeout=0.05) as ser:
            ...
            # CHANGED in 1.0.0(10): was a single fixed send_command_checked(
            # ser, CMD_SET_POWER_ALL, "SET POWER") built once from
            # config.json's "power" at process startup. On an FCC-certified
            # unit that value (33dBm, valid for CHINA units) gets rejected
            # with status 0x0105 - see send_power_command_with_fallback()
            # above for the full doc citation. Now tries the requested
            # value first and steps down automatically if rejected -
            # config.json is never touched, but a hard region cap no
            # longer silently leaves the module at an unconfirmed/stale
            # power level.
            if requested_power_dbm is not None:
                accepted_power_dbm, power_ok = send_power_command_with_fallback(
                    ser, ant_list_now, requested_power_dbm, send_command_checked
                )
            else:
                power_ok = send_command_checked(ser, CMD_SET_POWER_ALL, "SET POWER")
                accepted_power_dbm = requested_power_dbm  # unknown/unchanged this cycle
            ...
            # CHANGED in 1.0.0(10): was a single fixed send_command_checked(
            # ser, dwell_cmd, "SET ANTENNA DWELL") using whatever dwell
            # auto-tuning computed. On an FCC-certified unit a dwell value
            # that's perfectly fine on a CHINA unit (e.g. 800ms) can be
            # rejected with status 0x0500 - see
            # send_dwell_command_with_fallback() above for the full doc
            # citation. Now steps down automatically until the module
            # accepts one; if AUTO_TUNE is on and the accepted value
            # differs from the ideal one, one_cycle_s/QUIET_NEW_TAG are
            # recomputed against the REAL accepted dwell so the quiet-stop
            # logic later in this function stays consistent with what the
            # module is actually doing, not the value we originally asked
            # for.
            accepted_dwell_ms, dwell_ok = send_dwell_command_with_fallback(
                ser, ideal_dwell_ms, send_command_checked
            )
            if not dwell_ok:
                log.warning(
                    "[SCAN] SET ANTENNA DWELL not confirmed at any tried "
                    "value - module may fall back to its own 4000ms/antenna "
                    "default, which can make round-robin timing less "
                    "predictable within scan_timeout."
                )
            elif AUTO_TUNE and quiet_cycles_used is not None and accepted_dwell_ms != ideal_dwell_ms:
                ANTENNA_DWELL = accepted_dwell_ms
                one_cycle_s   = (antenna_count_now * accepted_dwell_ms) / 1000.0
                QUIET_NEW_TAG = max(one_cycle_s * max(1.0, quiet_cycles_used), one_cycle_s)
                QUIET_NEW_TAG = min(QUIET_NEW_TAG, max(MAX_SCAN_TIME - 0.5, one_cycle_s))
                log.info(
                    "[AUTO-TUNE] Recomputed after dwell fallback: dwell_ms=%d "
                    "(was %d) one_cycle=%.2fs quiet_stop=%.2fs",
                    accepted_dwell_ms, ideal_dwell_ms, one_cycle_s, QUIET_NEW_TAG
                )
            elif accepted_dwell_ms is not None:
                ANTENNA_DWELL = accepted_dwell_ms
            ...
            while True:
                ...
                for tag in tags:
                    ...
                    if tag["RSSI"] is not None:
                        ...
                        # NEW in 1.0.0(11): same idea, but broken down PER
                        # ANTENNA - answers "tag ini kebaca RSSI berapa di
                        # tiap antena" and "kalau kebaca di antena 1, apa
                        # antena 2 juga baca tag yang sama" (>1 key in the
                        # per-tag antenna map = yes). tag["ANT"] can be
                        # None if METADATA_FLAGS didn't request it, but
                        # this file always sets BIT2 (antenna) in
                        # METADATA_FLAGS=0x003F, so it's populated here.
    ...
    # NEW in 1.0.0(11): dump per-tag RSSI + per-antenna breakdown for this
    # cycle - see write_rssi_json() near power_cycle_before_first_scan()
    # for exactly what this writes. Only for PUBLISHED tags (same set as
    # result["epc_seen"] below), so it always matches what's actually
    # reported downstream - not raw/pre-filter reads.
    write_rssi_json(
        published_epc_seen, epc_antenna_rssi, epc_best_rssi,
        scan_meta={
            "timestamp":            time.strftime("%Y-%m-%d %H:%M:%S"),
            "scan_timeout_s":       MAX_SCAN_TIME,
            "duration_s":           duration,
