...
-Apps 1.0.0.(9)
# Added APPS_READY_FLAG path + write_ready_flag() function.
# On startup, deletes any stale flag from a previous run.
# Calls write_ready_flag() at the very end of startup (after broker detect/apply, bridge.conf, 
mosquitto restart, queue/binding) — right before serve().

-Send 1.0.0(9)
# Added wait_for_apps_ready() — polls for that flag every 1s, timeout 30s fallback.
# Added check_mosquitto_port() — quick extra socket check on 1883 (1s timeout).
# In __main__: deletes stale flag → starts apps.py → waits for ready flag → quick mosquitto check → 
then starts run/radar/light/switch.
