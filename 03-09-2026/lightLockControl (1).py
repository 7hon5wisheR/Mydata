#!/usr/bin/env python3
# -*- coding: utf-8 -*-
__version__ = "1.0.0(10) - restored start_dim_watcher() + owner lock file in main()"
...
#      Updated                    : 01-09-2026
#      Changes                    : main() was missing the call to
#                                   sublight.start_dim_watcher() (and the
#                                   /tmp/lightlockcontrol.lock owner-lock
#                                   file write) - a previous revert had
#                                   dropped it. Without that call, changing
#                                   "dim" in config.json (e.g. via apps.py's
#                                   /api/config or MQTT) while the light was
#                                   already On never re-applied the PWM
#                                   value live - the new dim only took
#                                   effect the next time the light was
#                                   turned Off then On again. Restored both
#                                   lines in main() below.
#      Author                     : Saifuddin
# ======================================================================================================================================
...
def main():
    ...
    # Start the in-process live "dim" watcher - this is what makes
    # changing "dim" in config.json (via apps.py /api/config or MQTT)
    # take effect immediately WHILE the light is already On. Without
    # this call, a new "dim" value only gets picked up the next time
    # the light is turned Off then On again (sub_on() reads it fresh,
    # but nothing else watches for changes in between).
    sublight.start_dim_watcher()

    # Write a lock file so light_pwm.py (manual bench-test tool)
    # refuses to run while this service - the real GPIO owner - is
    # active, preventing a second competing GPIO.PWM owner on pins
    # 16/18 (the root cause of the earlier flicker/blinking issue).
