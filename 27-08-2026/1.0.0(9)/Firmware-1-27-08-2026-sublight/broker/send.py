...
def cleanup_processes(*args):
    global process_rabbit, process_run, process_radar, process_light, process_switch, process_light_pwm
...
if __name__ == "__main__":
    ...
    try:
        ...
        # Small delay before starting light_pwm.py: it drives the GPIO/PWM
        # hardware directly (RPi.GPIO), so we let the previous module finish
        # claiming its pins/resources first to avoid a startup race.
        time.sleep(1)

        process_light_pwm, label = start_module("light_pwm.so", "light_pwm.py", "light_pwm")
        if process_light_pwm:
            print(f"{label} started")
