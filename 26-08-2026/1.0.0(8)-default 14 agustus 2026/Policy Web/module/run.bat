...
docker build -t utilityrftuning .
if errorlevel 1 (
...
docker rm -f utilityrftuning) >nul 2>&1
docker run -d --name utilityrftuning ^
  -p 5100:5100 ^
...
docker update --restart unless-stopped utilityrftuning
echo [3/4] Docker OK
