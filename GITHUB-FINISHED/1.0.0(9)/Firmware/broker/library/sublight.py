...
__version__ = "1.0.0(11) - lazy GPIO init, so importing this module no longer claims pins 16/18"
...
#      Updated                    : 28-08-2026
#      Changes                    : GPIO/PWM setup moved out of module import
#                                   time into a lazy _ensure_gpio() helper,
#                                   called only from functions that actually
#                                   drive the light (set_pwm/sub_on/sub_off/
#                                   sub_on_for/sub_off_for/start_dim_watcher).
#                                   Root cause fixed: run.py also does
#                                   `import sublight` (for get_light_status()
#                                   only) but the OLD module-level GPIO setup
#                                   ran unconditionally on import, so run.py's
#                                   process was silently creating a SECOND
#                                   GPIO.PWM object on pins 16/18 alongside
#                                   lightLockControl.py's - two independent
#                                   PWM signals fighting on the same physical
#                                   pin, which is what caused the blinking/
#                                   looping light. Lazy init means only the
#                                   process that actually calls a GPIO
#                                   function becomes the owner.
#      Author                     : Saifuddin
# ======================================================================================================================================
