...
Monitors IP address changes, then automatically:
  1. Changes the system hostname based on a mapping of the last IP octet -> letter
  2. Updates the clientid & topic lines in bridge.conf
  3. Restarts the mosquitto (broker) service so the config gets reloaded

Mapping of last octet -> letter (BASE_OCTET=100):
...
CHECK_INTERVAL = 5                 # seconds, interval between IP checks

# IMPORTANT: the service runs as root, so don't use "~".
...
def get_current_ip(interface: str) -> Optional[str]:
    """Get the current active IP address of a given interface."""
    try:
...
def octet_to_letter(last_octet: int) -> Optional[str]:
    ...
    if idx < 1 or idx > 26:
        log.warning(f"Octet {last_octet} is out of mapping range (101-126)")
        return None
...
def update_bridge_conf(path: str, new_id: str) -> bool:
    """
    Update the 3 lines in bridge.conf that use the device name (all sharing the same value):
    ...
    if changed:
        ...
        log.info("bridge.conf already up to date, no changes made")
    return changed


def reboot_pi(delay: int = 5):
    """Reboot the Raspberry Pi. Called after hostname & bridge.conf have been updated,
    so that all services (including apps.py, which reads the hostname once at startup)
    also get refreshed. A short delay is given so logs & files have time to flush first."""
...
def process_ip_change(ip: str):
    ...
    # Save the IP before rebooting, so that after coming back online the
...
def main():
    log.info("ipservice.py started, monitoring for IP changes...")
